...
const ShoppingCartButton = (props) => {
  ...
  const buttonClasses = `relative h-10 w-10 hover:scale-110 hover:translate-y-1 transition-all duration-150 ${
    buttonAnimate ? "scale-110 ease-in ease-out" : ""
  }`;
  ...
  return (
    <button className={buttonClasses} onClick={props.onClick}>
      <ShoppingCartIcon />
      <div className="rounded-full bg-Red text-yellow-200 text-xs font-semibold absolute top-0 right-0 max-w-full min-w-4">
        {numOfCartItems}
      </div>
    </button>
  );
